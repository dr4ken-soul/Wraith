...
export function Mechanism() {
  useEffect(() => {
    ...
    const clearHashAfterLeaving = new IntersectionObserver(([entry]) => {
      if (!entry.isIntersecting && window.location.hash === '#mechanism') {
        window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}`)
      }
    }, {threshold: 0.01})

    clearHashAfterLeaving.observe(section)
    return () => clearHashAfterLeaving.disconnect()
  }, [])
